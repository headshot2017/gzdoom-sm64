...
TriMatrix TriMatrix::frustum(float left, float right, float bottom, float top, float near_, float far_)
{
	float a = (right + left) / (right - left);
	float b = (top + bottom) / (top - bottom);
	float c = -(far_ + near_) / (far_ - near_);
	float d = -(2.0f * far_) / (far_ - near_);
	TriMatrix m = null();
	m.matrix[0 + 0 * 4] = 2.0f * near_ / (right - left);
	m.matrix[1 + 1 * 4] = 2.0f * near_ / (top - bottom);
	m.matrix[0 + 2 * 4] = a;
	m.matrix[1 + 2 * 4] = b;
	m.matrix[2 + 2 * 4] = c;
	m.matrix[2 + 3 * 4] = d;
